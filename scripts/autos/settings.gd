...
## Returns the value of [param section]:[param key] if it's defined in the config,
## otherwise sets [param section]:[param key] to [param value] and
## returns [param value]
func load_value(section : String, key : String, value : Variant) -> Variant:
	if has_section_key(section, key):
		var kvalue : Variant = get_value(section, key)
		if typeof(kvalue) == typeof(value):
			return kvalue
